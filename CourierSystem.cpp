...
void CourierSystem::viewAllRoutes(int srcID, int destID)
{
    ...
    function<void(int, int, int)> dfs = [&](int current, int target, int currentDist)
    {
        ...
        if (visited[current] || currentPath.size() > 10)
            return;
...
void CourierSystem::displayParcelsByWeight()
{
    ...
    {
        t.addRow({to_string(sorted[i].trackingID),
...
void CourierSystem::displayRiders()
{
    ...
    for (int i = 0; i < riders.size(); i++)
    {
        ...
        t.addRow({to_string(riders[i].riderID),
                  riders[i].name,
    ...
    for (int i = 0; i < riders.size(); i++)
    {
        if (riders[i].riderID == riderID)
        {
            ...
            for (int j = 0; j < riders[i].assignedParcels.size(); j++)
            {
                ...
                if (parcel)
                {
                    t.addRow({to_string(parcel->trackingID),
...
void CourierSystem::displayOperationLog()
{
    ...
    for (int i = logs.size() - 1; i >= 0; i--)
    {
        ...
        case OperationType::AddParcel:
            ...
            break;
        case OperationType::RemoveParcel:
            opType = "Remove Parcel";
            break;
        case OperationType::ProcessParcel:
            opType = "Process Parcel";
            break;
        case OperationType::AssignRider:
            opType = "Assign Rider";
            break;
        case OperationType::CompleteDelivery:
            opType = "Complete Delivery";
            break;
        case OperationType::BlockRoute:
            opType = "Block Route";
            break;
        case OperationType::UnblockRoute:
            opType = "Unblock Route";
            break;
        case OperationType::ReturnToSender:
            opType = "Return to Sender";
            break;
        }

        t.addRow({logs[i].timestamp,
                  opType,
...
void CourierSystem::saveOperationLog(const string &filename)
{
    ...
    for (int i = logs.size() - 1; i >= 0; i--)
    {
        ...
        case OperationType::RemoveParcel:
            ...
            break;
        case OperationType::ProcessParcel:
            opType = "ProcessParcel";
            break;
        case OperationType::AssignRider:
            opType = "AssignRider";
            break;
        case OperationType::CompleteDelivery:
            opType = "CompleteDelivery";
            break;
        case OperationType::BlockRoute:
            opType = "BlockRoute";
            break;
...
void CourierSystem::loadOperationLog(const string &filename)
{
    ...
    while (getline(file, line))
    {
        ...
        if (opStr == "AddParcel")
            opType = OperationType::AddParcel;
        else if (opStr == "RemoveParcel")
            opType = OperationType::RemoveParcel;
        else if (opStr == "ProcessParcel")
            opType = OperationType::ProcessParcel;
        else if (opStr == "AssignRider")
            opType = OperationType::AssignRider;
        else if (opStr == "CompleteDelivery")
            opType = OperationType::CompleteDelivery;
        else if (opStr == "BlockRoute")
            opType = OperationType::BlockRoute;
        else if (opStr == "UnblockRoute")
            opType = OperationType::UnblockRoute;
        else if (opStr == "ReturnToSender")
            opType = OperationType::ReturnToSender;
        else
            continue;
    ...
    if (parcel->currentRoute.size() > 0)
    {
        ...
        for (int i = 0; i < parcel->currentRoute.size(); i++)
        {
            ...
            if (c)
                ...
                cout << parcel->currentRoute[i];

            if (i < parcel->currentRoute.size() - 1)
                cout << " -> ";
        }
        cout << "\n";
...
void CourierSystem::displayAllParcels()
{
    ...
    for (int i = 0; i < allParcels.size(); i++)
    {
        ...
        t.addRow({to_string(p.trackingID),
                  p.senderName,
                  p.receiverName,
                  p.getPriorityStr(),
...
void CourierSystem::displayDeliveryStatistics()
{
    ...
    for (int i = 0; i < allParcels.size(); i++)
    {
        if (allParcels[i].status == Status::Delivered)
            ...
            inTransit++;
        else if (allParcels[i].status == Status::Pending)
            pending++;
        else if (allParcels[i].status == Status::Returned)
            returned++;

        if (allParcels[i].priority == Priority::Overnight)
            overnight++;
        else if (allParcels[i].priority == Priority::TwoDay)
            twoDay++;
        else if (allParcels[i].priority == Priority::Normal)
            normal++;
    }
...
void CourierSystem::initializeAdmins()
{
    ...
    for (int i = 0; i < admins.size(); i++)
    {
        if (admins[i].username == "admin" && admins[i].isSuperAdmin)
        {
            adminExists = true;
            break;
        }
    ...
    if (!adminExists)
    {
        admins.push_back(Admin("admin", "admin123", true));
    }
}
